...
class Usertype extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
                    array('usertype_name', 'required'),
			array('financialyearid, companyid', 'numerical', 'integerOnly'=>true),
			array('usertype_name', 'length', 'max'=>45),
                     array('usertype_name','unique','message'=>'{attribute}:{value} already exists!'),
             array('usertype_name', 'match',
                     'pattern' => '/^[a-zA-Z]{1}[a-zA-Z0-9 _.-]*$/',
                     'message' => 'User Type name should contain only alphabets, numbers, space, dot, hyphen and underscore and first character must be alphabet.'),
            // The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('usertypeid, usertype_name, financialyearid, companyid', 'safe', 'on'=>'search'),
		);
	}
...
$financialyear = Financialyear::model()->findByAttributes(array('status' => 1));  //! $financialyear stores active financial year details
		...
		$criteria->compare('usertype_name',$this->usertype_name,true);
