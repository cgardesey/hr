...
class WorkshiftController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','Gethoursperday'),
				'users'=>array('@'),
			),
        ...
        public function actionGethoursperday(){
            $starttime = $_POST['starttime'];
            $endtime = $_POST['endtime'];
            $to_time = strtotime($starttime);
                $from_time = strtotime($endtime);
                $sum = (float) round(abs($to_time - $from_time) / 60, 2);
                $hours = (float) $sum / 60;
                echo $hours;
        }
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['Workshift'];
...
//                    echo $error;
	 ...
	 * @param Workshift $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='workshift-form')
		{
